...
from utils import sample_box_pose, sample_box_pose_realman, sample_insertion_pose # robot functions
...
def main(args):
    ...
    if not is_eval:
        wandb.init(project="mobile-aloha2", reinit=True, entity="qindeyun24", name=expr_name)
...
def eval_bc(config, ckpt_name, save_episode=True, num_rollouts=50):
    ...
    if real_robot:
        ...
        # env_max_reward = 0
        cprint('Real robot is used, please make sure the robot is ready!', 'green')
    else:
        # from sim_env import make_sim_env
        # env = make_sim_env(task_name)
        # env_max_reward = env.task.max_reward
        cprint('Sim environment is used', 'green')
    ...
    for rollout_id in range(num_rollouts):
        ...
        if 'sim_push_cube' in task_name:
            BOX_POSE[0] = sample_box_pose_realman()
